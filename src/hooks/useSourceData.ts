...
import { AOIJSON, useAOI } from '@/store/useDrawStore/useDrawStore';
...
export const useSourceDataFetch = (params?: SourceGeoJSONQueryParameters) => {
    ...
    const { data, error, isLoading } = useSWRImmutable<MapTypes.SourceData>(
        { url: API_ENDPOINT, ...userGroupParams, ...params },
        httpGet
    );
...
export const useAPIFilteredSourceData = () => {
    ...
    const [{ date, instrument, plume_status, plume_qualities, eps }] = usePortalQueryParams();
...
export function getFeaturesInsideAOI<T extends Feature>(aoi: AOIJSON | null, features: T[]) {
    ...
    const points = features.filter(feature => {
        ...
        const { coordinates } = geometry as { coordinates: turf.Position };
