...
import { dateParam2Dates, toggleArrayValues, type Params } from '@/utils/usePortalQueryParams';
...
import { useTrackedPortalQueryParams } from './useTrackedPortalQueryParams';
...
export const useFilterTags = ({ params, hasAoi }: FilterTags) => {
    ...
    const sectorTags = useMemo(() => {
        ...
        return sector.map(sectorKey => ({
            ...
            onClick: () => onCloseSourceDetail({ sector: toggleArrayValues(sectorKey, sector) }) // could abstract
        }));
    ...
    const instrumentTags = useMemo(() => {
        ...
        return instrument.map(instrumentKey => ({
            ...
            onClick: () => onCloseSourceDetail({ instrument: toggleArrayValues(instrumentKey, instrument) }) // could abstract
        }));
    ...
    const plumeQualityTags = useMemo<FiltersTagItem[]>(
        () =>
            plumeQualities.map(plumeQuality => ({
                ...
                onClick: () => onCloseSourceDetail({ plume_qualities: toggleArrayValues(plumeQuality, plumeQualities) }) // could abstract
            })),
    ...
    const singleTags = useMemo(
        () => ({
            coordinates: getCoordinateTag(coordinates, () => onCloseSourceDetail({ coordinates: null })),
            location: getLocationTag(location, () => onCloseSourceDetail({ location: null })),
            date: getDateTag(date_start, date_end, () => onCloseSourceDetail({ date: null })),
            gasType: getGasTypeTag(gasType, () => onCloseSourceDetail({ gasType: null })),
            ...
            plumeStatus: plumeStatus
                ? {
                      ...
                      onClick: () => onCloseSourceDetail({ plume_status: undefined })
                  }
                : null,
            aoi: getAOITag(hasAoi, () => deleteAOI())
        }),
        [
            coordinates,
            location,
...
export const useCurrentFilterTags = () => {
    const [data] = useTrackedPortalQueryParams();
    const aoi = useAOI();
