...
export const ObservationsList = ({ data, sourceCoordinates }: Props) => {
    ...
    const onArrowMove = (direction: number): void => {
        ...
        const plumeLayerId = activePlumeLayerId?.slice(6, activePlumeLayerId.length);
        const currentIndex = items.findIndex(item => item.id === plumeLayerId);
        ...
        const nextPlumeLayerId = `plume-${nextPlumeId}`;
        ...
        if ('plume_bounds' in nextPlumeItem) {
            const plumeImageObject = {
                plumeBounds: nextPlumeItem.plume_bounds,
                plumeUrl: nextPlumeItem.plume_png,
                plumeId: nextPlumeItem.id
            };
        ...
        nextPlumeCoords && easeTo(nextPlumeCoords, 'details');
    };
