...
const Sources = () => {
    ...
    const { data: sources } = useSourcesInView();
    ...
    const getMoreItems = useCallback(
        () =>
            setLimit(previous => {
                if (previous < itemCount) return previous + defaultLimit;
                return previous;
            }),
        [itemCount]
    );
    const throttledGetMoreItems = useMemo(() => throttle(getMoreItems, 100), [getMoreItems]);
    ...
    return (
        <motion.div className={styles.wrapper} {...animationVariant}>
            ...
            <SourcesList
                ...
                items={listItems}
                limit={limit}
                getMoreItems={throttledGetMoreItems}
                onReset={resetLimit}
            />
        </motion.div>
    );
