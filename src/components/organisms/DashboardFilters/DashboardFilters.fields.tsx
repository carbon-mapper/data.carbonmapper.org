...
import { usePortalQueryParams, toggleArrayValues } from '@/utils/usePortalQueryParams';
...
export const SectorToggle = ({ onClick }: DashboardFiltersTypes.FieldToggleProps) => {
    const [{ sector }] = usePortalQueryParams();
...
export const InstrumentToggle = ({ onClick }: DashboardFiltersTypes.FieldToggleProps) => {
    const [{ instrument }] = usePortalQueryParams();
...
export const PlumeStatusToggle = ({ onClick }: DashboardFiltersTypes.FieldToggleProps) => {
    const [{ plume_status: plumeStatus }] = usePortalQueryParams();
...
export const PlumeQualityToggle = ({ onClick }: DashboardFiltersTypes.FieldToggleProps) => {
    const [{ plume_qualities: plumeQualities }] = usePortalQueryParams();
...
export const SectorList = () => {
    ...
    return (
        <FilterSelectList
            ...
            onAllItemClick={() => {
                setParams({ sector: [] });
                trackSearchSubmitEvent('sector all', {
                    ...params,
                    sector: []
                });
            }} // This function takes a param we don't need
        />
    );
};
...
export const InstrumentList = () => {
    ...
    const instrumentDetails = Object.entries(INSTRUMENT_MAP).map(([key, value]) => ({
        ...
        onClick: () => {
            ...
            trackSearchSubmitEvent('instrument', {
                ...params,
                instrument: newValues
            });
        }
    }));
    ...
    return (
        <FilterSelectList
            ...
            onAllItemClick={() => {
                ...
                trackSearchSubmitEvent('instrument all', {
                    ...params,
                    instrument: []
                });
            }}
...
export const PlumeStatusList = () => {
    const [params, setParams] = usePortalQueryParams();
    const { plume_status: plumeStatus } = params;
    ...
    const items = Object.entries(PLUME_STATUS_MAP).map(([key, value]) => ({
        ...
        onClick: () => {
            setParams({ plume_status: key });
            trackSearchSubmitEvent('plume status', {
                ...params,
                plume_status: key
            });
        }
    ...
    return (
        <FilterSelectList
            ...
            onAllItemClick={() => {
                ...
                trackSearchSubmitEvent('plume status all', {
                    ...params,
...
export const PlumeQualityList = () => {
    const [params, setParams] = usePortalQueryParams();
    const { plume_qualities: plumeQualities } = params;
    ...
    const item = Object.entries(PLUME_QUALITY_MAP).map(([key, value]) => ({
        ...
        onClick: () => {
            ...
            trackSearchSubmitEvent('plume quality', {
                ...params,
                plume_qualities: newValues
            });
        }
    }));
    ...
    return (
        <FilterSelectList
            ...
            onAllItemClick={() => {
                ...
                trackSearchSubmitEvent('plume quality all', {
                    ...params,
                    plume_qualities: []
                });
            }}
...
export const Checkboxes = () => {
    const [params, setParams] = usePortalQueryParams();
    const { gasType, emission_max, plume_max } = params;
    ...
    const gasTypeHandler = (value: GasType) => {
        ...
        if (gasType === undefined) {
            ...
            trackSearchSubmitEvent('gas type', { ...params, gasType: newSelectedGasType });
        } else {
            setParams({ gasType: undefined });
            trackSearchSubmitEvent('gas type all', { ...params, gasType: undefined });
        }
    };
...
export const Ranges = () => {
    ...
    const [params, setParams] = usePortalQueryParams();
    const { emission_min, emission_max, plume_min, plume_max, persistence_min, persistence_max } = params;
    ...
    return (
        <FilterFieldset legend='Advanced range filters'>
            <FilterRange
                ...
                isPercentage={false}
                onChange={([from, to]) => {
                    const emission_min = from === 0 ? undefined : from;
                    const emission_max = to === combinedMaxEmissions ? undefined : to;

                    setParams({ emission_min, emission_max });
                    trackSearchSubmitEvent('emission rate', { ...params, emission_min, emission_max });
                }}
                label={{
            ...
            <FilterRange
                ...
                isPercentage={false}
                onChange={([from, to]) => {
                    const plume_min = from === 0 ? undefined : from;
                    const plume_max = to === combinedMaxPlumes ? undefined : to;
                    setParams({ plume_min, plume_max });
                    trackSearchSubmitEvent('plume count', { ...params, plume_min, plume_max });
                }}
                label={{
            ...
            <FilterRange
                ...
                isPercentage={true}
                onChange={([from, to]) => {
                    const persistence_min = from === 0 ? undefined : from;
                    const persistence_max = to === MAX_PERSISTENCE_LIMIT ? undefined : to;
                    setParams({ persistence_min, persistence_max });
                    trackSearchSubmitEvent('source persistence', { ...params, persistence_min, persistence_max });
                }}
                label={{
...
export const CalendarToggle = ({ onClick }: DashboardFiltersTypes.FieldCalendarToggleProps) => {
    const [{ date }] = usePortalQueryParams();
...
export const CalendarItem = ({ type }: DashboardFiltersTypes.CalendarItemProps) => {
    ...
    const onChange = (newDate: FiltersDate) => {
        ...
        setParams({ date });
        trackSearchSubmitEvent('date', { ...params, date });
    };
